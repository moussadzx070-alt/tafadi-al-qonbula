...
QComboBox, QSpinBox, QLineEdit { background:#0E1727; border:1px solid #344A6C; border-radius:9px; padding:10px; color:#FFF; }
    ...
    def play(self, name: str) -> None:
        ...
        if path.exists():
            try:
                ...
                effect.play()
                self._last_effect = effect # الحفاظ على المرجع لمنع التقطيع
            except ImportError:
                pass # تجاهل الصوت في حال تم استبعاد مكتبة Multimedia أثناء البناء لتقليص الحجم
...
class StartScreen(QWidget):
    ...
    def __init__(self):
        ...
        box.setContentsMargins(45, 45, 45, 45)
        box.setSpacing(18)
        
        try:
            emblem = QLabel(); emblem.setPixmap(Assets.pixmap("bomb.svg", 112, 112)); emblem.setAlignment(Qt.AlignCenter)
            box.addWidget(emblem)
        except:
            pass # تجاوز الخطأ إذا كانت الأيقونة غير موجودة
        ...
        box.addWidget(title); box.addWidget(sub); box.addSpacing(12)
        
        play = QPushButton("ابدأ اللعبة"); play.setObjectName("primary"); play.clicked.connect(self.start_requested.emit)
        settings = QPushButton("الإعدادات"); settings.clicked.connect(self.settings_requested.emit)
...
class SetupScreen(QWidget):
    ...
    def __init__(self):
        ...
        head = QHBoxLayout()
        back = QPushButton("رجوع"); back.clicked.connect(self.back_requested.emit)
        head.addWidget(back); head.addStretch()
        title = QLabel("إعداد اللاعبين"); title.setObjectName("title")
        head.addWidget(title); root.addLayout(head)
        
        form = QFrame(); form.setObjectName("panel")
        fl = QVBoxLayout(form)
        
        row = QHBoxLayout()
        row.addWidget(QLabel("عدد اللاعبين"))
        self.count = QSpinBox(); self.count.setRange(2, 24); self.count.setValue(4)
        self.count.valueChanged.connect(self._refresh)
        row.addWidget(self.count); row.addStretch()
        fl.addLayout(row)
        
        self.scroll = QScrollArea(); self.scroll.setWidgetResizable(True)
        self.names_widget = QWidget()
        self.grid = QGridLayout(self.names_widget)
        self.scroll.setWidget(self.names_widget)
        fl.addWidget(self.scroll)
        
        start = QPushButton("ابدأ الدورة الأولى"); start.setObjectName("primary")
    ...
    def _refresh(self, count: int):
        ...
        for i in range(count):
            label = QLabel(f"اللاعب {i+1}")
            edit = QLineEdit(f"لاعب {i+1}")
            edit.setPlaceholderText("اكتب اسم اللاعب")
    ...
    def _submit(self):
        ...
        if any(not n for n in names): 
            QMessageBox.warning(self, "بيانات ناقصة", "يرجى إدخال أسماء جميع اللاعبين.")
            return
...
class GameScreen(QWidget):
    ...
    def __init__(self, engine: GameEngine, audio: AudioSystem):
        super().__init__()
        self.engine = engine; self.audio = audio
        self.cards = []
        
        root = QVBoxLayout(self); root.setContentsMargins(24, 20, 24, 20); root.setSpacing(12)
        top = QHBoxLayout()
        home = QPushButton("الرئيسية"); home.clicked.connect(self.home_requested.emit)
        top.addWidget(home); top.addStretch()
        
        self.cycle_label = QLabel(); self.cycle_label.setObjectName("section")
        top.addWidget(self.cycle_label); top.addStretch()
        self.progress = QLabel()
        top.addWidget(self.progress); root.addLayout(top)
        
        self.bomb_label = QLabel("موقع القنبلة مخفي"); self.bomb_label.setObjectName("muted"); self.bomb_label.setAlignment(Qt.AlignCenter)
        root.addWidget(self.bomb_label)
        
        self.grid = QGridLayout()
        root.addLayout(self.grid, 1)
        
        self.action_panel = QFrame(); self.action_panel.setObjectName("panel")
        self.actions = QHBoxLayout(self.action_panel)
        ...
        if self.engine.state in {GameState.ROUND_1, GameState.ROUND_2, GameState.ROUND_3}:
            ...
            if active:
                ...
                for text, decision in (("البقاء في مكاني", Decision.STAY), ("العودة إلى مكاني الأصلي", Decision.HOME)):
                    b = QPushButton(text)
                    b.clicked.connect(lambda _, d=decision, pid=active.player_id: self._choose(pid, d))
                    self.actions.addWidget(b)
                    
                move = QPushButton("أخذ مكان لاعب حي آخر")
                move.clicked.connect(lambda: self._choose_target(active))
                self.actions.addWidget(move)
                
            if self.engine.all_decisions_submitted(): 
                self._add_commit()
                
        elif self.engine.state == GameState.READY_TO_REVEAL:
            self.status.setText("اكتملت الاختيارات. المشرف وحده يكشف القنبلة.")
            reveal = QPushButton("إظهار القنبلة"); reveal.setObjectName("danger")
            reveal.clicked.connect(self._reveal)
            self.actions.addWidget(reveal)

    def _choose(self, pid, decision, target_id=None):
        self.engine.submit_decision(pid, decision, target_id)
        self.audio.play("decision")
        self._render()

    def _choose_target(self, active):
        dialog = QDialog(self)
        dialog.setWindowTitle("اختر لاعبًا حيًا")
        layout = QVBoxLayout(dialog)
        combo = QComboBox()
        
        for p in self.engine.alive_players:
            if p.player_id != active.player_id: 
                combo.addItem(p.name, p.player_id)
                
        if combo.count() == 0:
            QMessageBox.information(self, "غير متاح", "لا يوجد لاعبون آخرون أحياء لأخذ مكانهم.")
            return
        ...
        if dialog.exec():
            selected_target_id = combo.currentData()
            self._choose(active.player_id, Decision.MOVE_TO_PLAYER, selected_target_id)

    def _add_commit(self):
        commit = QPushButton("تثبيت الجولة"); commit.setObjectName("primary")
        commit.clicked.connect(lambda: (self.engine.commit_round(), self.audio.play("round"), self._render()))
        self.actions.addWidget(commit)

    def _reveal(self):
        self.setEnabled(False)
        self.bomb_label.setText("جاري كشف موقع القنبلة...")
        effect = QGraphicsOpacityEffect(self.bomb_label)
        self.bomb_label.setGraphicsEffect(effect)
        anim = QPropertyAnimation(effect, b"opacity", self)
        anim.setDuration(900)
        anim.setStartValue(0.15)
        anim.setEndValue(1.0)
        anim.setEasingCurve(QEasingCurve.InOutQuad)
        anim.finished.connect(self._finish_reveal)
        anim.start()
        self._anim = anim

    def _finish_reveal(self):
        victim = self.engine.reveal_bomb()
        self.bomb_label.setText(f"القنبلة كانت في المكان {self.engine.bomb_slot}")
        
        if victim: 
            QMessageBox.warning(self, "انفجار", f"خرج اللاعب: {victim.name}")
            
        if self.engine.alive_count <= 1:
            winner = self.engine.get_winner()
            self.setEnabled(True)
            QMessageBox.information(self, "الفائز", winner.name if winner else "لا يوجد فائز")
            self.finished.emit()
            return
            
        self.engine.start_next_cycle()
        self.setEnabled(True)
        self._render()
...
class SettingsDialog(QDialog):
    def __init__(self, audio: AudioSystem, parent=None):
        ...
        self.sound = QComboBox(); self.sound.addItems(["مفعّل", "متوقف"]); self.sound.setCurrentIndex(0 if audio.enabled else 1)
    ...
    def _save(self): 
        self.audio.enabled = self.sound.currentIndex() == 0
        self.audio.effects = self.effects.currentIndex() == 0
        self.accept()

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("تفادي القنبلة")
        ...
        self.start.start_requested.connect(lambda: self.stack.setCurrentWidget(self.setup))
        self.start.settings_requested.connect(self._settings)
        self.start.about_requested.connect(self._about)
        self.setup.back_requested.connect(lambda: self.stack.setCurrentWidget(self.start))
        self.setup.started.connect(self._start_game)
        
    def _settings(self): 
        SettingsDialog(self.audio, self).exec()
        
    def _about(self): 
        QMessageBox.information(self, "حول اللعبة", "تفادي القنبلة\nلعبة جماعية عربية تعمل بالكامل دون اتصال بالإنترنت.")
        
    def _start_game(self, names):
        self.engine = GameEngine(names)
        self.engine.start_game()
        self.game = GameScreen(self.engine, self.audio)
        self.stack.addWidget(self.game)
        self.stack.setCurrentWidget(self.game)
        self.game.home_requested.connect(lambda: self.stack.setCurrentWidget(self.start))
        self.game.finished.connect(lambda: self.stack.setCurrentWidget(self.start))
